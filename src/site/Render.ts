...
<main><section class="run-header" aria-label="CI run"><div class="run-title"><h1>Test results</h1><span class="run-state ${model.complete ? "healthy" : "attention"}">${model.complete ? "Complete" : "Needs attention"}</span></div><div class="run-meta"><a href="${runUrl}">Run #${escapeHtml(run.number)} ↗</a><span>${escapeHtml(run.branch)}</span><a class="mono" href="${repo}/commit/${encodeURIComponent(run.sha)}">${escapeHtml(run.sha.slice(0, 8))}</a><span>Attempt ${escapeHtml(run.attempt)}</span><time datetime="${escapeHtml(model.generatedAt)}" title="Generated">${escapeHtml(model.generatedAt.replace("T", " ").slice(0, 16))} UTC</time></div></section>
<section class="metrics" aria-label="Result totals"><div><span class="metric-label">Results</span><strong>${total}<small> / ${total + missing}</small></strong></div><div><span class="metric-label">Passed</span><strong class="green">${counts.pass ?? 0}</strong></div><div><span class="metric-label">Accepted exceptions</span><strong class="amber">${exceptions}</strong><p>${counts.divergence ?? 0} divergences · ${counts["known-bug"] ?? 0} known bugs</p></div><div><span class="metric-label">Failed / missing</span><strong class="${failures + missing ? "red" : "muted"}">${failures + missing}</strong><p>${failures} failed · ${missing} missing</p></div></section>
<section class="suites-section" aria-label="CI suite status"><div class="section-heading"><h2>Suites</h2></div><div class="suite-grid">${model.suites.map((suite) => `<details class="suite-card ${suite.status}"><summary><span class="sr-only">${escapeHtml(suite.status)}: </span><span class="suite-indicator" aria-hidden="true"></span><span>${escapeHtml(suite.label)}<small>${suite.expected ? `${suite.report?.cases.filter((result) => result.status === "pass").length ?? 0} passed · ${suite.expected} scheduled` : "Formatting · types · unit tests"}</small></span><span class="chevron" aria-hidden="true">⌄</span></summary><div class="suite-detail"><p>Job: <strong>${escapeHtml(suite.conclusion)}</strong></p>${suite.problems.map((problem) => `<p class="problem">${escapeHtml(problem)}</p>`).join("")}${suite.report ? `<p><a href="./reports/${suite.key}.json" download>Download report JSON ↓</a></p><p class="mono">${escapeHtml(suite.report.runId)}</p>` : ""}</div></details>`).join("")}</div></section>
...
<div class="matrix-meta"><span id="visible-count" role="status">${model.rows.length} cases</span><span>Click a result for details <span aria-hidden="true">↗</span></span></div><div class="table-scroll"><table><thead><tr><th scope="col">Case / contract</th><th scope="col">Suite</th><th scope="col">Reference <small>workerd × 2</small></th><th scope="col">Candidate <small>celld</small></th></tr></thead><tbody>${model.rows.map((row, index) => `<tr data-row="${index}"><th scope="row"><span class="family">${escapeHtml(row.family)}</span><span class="case-name">${escapeHtml(row.id.slice(row.family.length + 1))}</span>${row.contract && /^https:\/\//.test(row.contract) ? `<a class="contract-link" href="${escapeHtml(row.contract)}" aria-label="Read contract for ${escapeHtml(row.id)}">↗</a>` : ""}</th><td><span class="suite-name">${escapeHtml(groupLabels[row.group] ?? row.group)}</span></td><td>${cellHtml(row.reference, index, "reference")}</td><td>${cellHtml(row.candidate, index, "candidate")}</td></tr>`).join("")}</tbody></table></div><div id="empty" hidden class="empty"><strong>No matching cases</strong><button id="clear">Clear filters</button></div><noscript><p>All cases are shown. Enable JavaScript to filter results and inspect individual observations; report downloads work without it.</p></noscript>
