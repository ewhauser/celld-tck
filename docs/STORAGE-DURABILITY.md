...
**Observed on celld v0.5.0** (run `tck-2ccec1c3-ebdd-4cad-b857-9f09bcd28721`): the barrier neither rejects nor blocks. 156 rounds all resolved in 2–12 ms, and the owner's log shows 29 `proof="fleet"` waits before the cut followed by 137 `proof="bucket"` waits during it — the documented single-node fallback to bucket durability, not a false claim of replication. After reconnection and a full fleet restart, all three nodes reported the last acknowledged barrier value (`barrier-156`) and exactly the seeded SQL row. This is documented behaviour, so no bug is registered.
